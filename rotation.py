...
class SphericalImage:
    def __init__(self, equImage):
        ...
        phi, theta = np.meshgrid(
            np.linspace(0, np.pi, num=self.__dim[0], endpoint=False),
            np.linspace(0, 2 * np.pi, num=self.__dim[1], endpoint=False),
        )
        self.__coordSph = np.stack(
            [(np.sin(phi) * np.cos(theta)).T, (np.sin(phi) * np.sin(theta)).T, np.cos(phi).T], axis=2,
        )
    ...
    def rotate(self, R):
        ...
        self.mapped = np.stack(
            [theta.reshape(self.__dim[0], self.__dim[1]), phi.reshape(self.__dim[0], self.__dim[1])], axis=2,
        ).astype(np.float32)
